...
/**
 * @extends \Illuminate\Database\Eloquent\Factories\Factory<\App\Models\SapOperation>
 */
...
//        $operationTypes = OperationType::where('name', '=', SapOperation::$sap_operation_type_name)->get('id');
        $operationTypes = OperationType::pluck('id')->random();
        $accountSapId = Account::pluck('sap_id')->random();

        return [
            'operation_type_id' => $operationTypes,
            'title' => fake()->text(20),
            'date' => fake()->date,
            'subject' => fake()->name,
            'sum' => fake()->randomFloat(2,1,1000),
            'sap_id' => fake()->randomNumber(5),
            'account_sap_id' => $accountSapId
            /*
            HINT:
